...
template <size_t K, class KeyType, class ValType> class kdtree {
  size_t dimension = K;
  size_t _size = 0;
  struct node {
    node *parent = NULL;
...
public:
  ...
  public:
    ...
    const vector<KeyType> &key() const {
      return n->key;
    } // modification is not available
    ValType &val() {
      return n->val;
    } // modification doesn't yield invalid tree structure
  };

  void insert(vector<pair<vector<KeyType>, ValType>> &records);
  void insert(const vector<KeyType> &key, const ValType &val);
  void _recursiveInserter(
      vector<vector<pair<vector<KeyType>, ValType> *>> &sortedArrays,
      size_t split = 0);
  ostream &print(ostream &o = std::cout) const {
    _postorder(o, root);
    return o;
  }
  iterator nearest_neighbour(const vector<KeyType> &test_point,
                             ValType val) const;
  vector<iterator> n_nearest_neighbour(const size_t &n,
                                       const vector<KeyType> &test_point,
                                       ValType val) const;
};

template <size_t K, class KeyType, class ValType>
void kdtree<K, KeyType, ValType>::insert(
    vector<pair<vector<KeyType>, ValType>> &records) {
  ...
  for (int i = 0; i < K; i++) {
    ...
    auto comp = [&](Record *l_val, Record *r_val) {
      auto j = i;
      if (!(l_val && r_val))
        throw internal_error("NULL");
      // cycle through K dimensions to find comparable coordinate
      while ( // Test coord equality
          !(l_val->first[j % K] < r_val->first[j % K] ||
            r_val->first[j % K] < l_val->first[j % K])
          // Test bounds
          && j < i + K)
        j++;
...
// I <3 Templates
template <size_t K, class KeyType, class ValType>
void kdtree<K, KeyType, ValType>::_recursiveInserter(
    vector<vector<pair<vector<KeyType>, ValType> *>> &sortedArrays,
    size_t split) {
  if (sortedArrays.size() != K)
    throw invalid_dimension();
...
template <size_t K, class KeyType, class ValType>
const KeyType kdtree<K, KeyType, ValType>::sq_distance(const node &X,
                                                       const node &Y) const {
  /*
...
template <size_t K, class KeyType, class ValType>
ostream &kdtree<K, KeyType, ValType>::_postorder(ostream &o, node *p,
                                                 int indent) const {
  if (p != NULL) {
    if (p->right) {
...
template <size_t K, class KeyType, class ValType>
void kdtree<K, KeyType, ValType>::insert(const vector<KeyType> &key,
                                         const ValType &val) {
  if (key.size() != K)
    throw invalid_dimension();
...
kdtree<K, KeyType, ValType>::nearest_neighbour(
    const vector<KeyType> &test_point, ValType val) const {
  return n_nearest_neighbour(1, test_point, val)[0];
}

template <size_t K, class KeyType, class ValType>
vector<typename kdtree<K, KeyType, ValType>::iterator>
kdtree<K, KeyType, ValType>::n_nearest_neighbour(
    const size_t &n, const vector<KeyType> &test_point, ValType val) const {
  using dist_node = pair<KeyType, node *>;
  priority_queue<dist_node> q;

  if (n > _size)
    throw internal_error("Tobb a lekerdezes mint az elem!");
  if (test_point.size() != K)
    throw invalid_dimension();
  if (root == NULL)
    throw empty_tree();
  ...
  while (!s.empty()) {
    ...
    if (state == unwinding) {
      ...
      auto hyper_rad =
          curr->key[curr->split_index] - query.key[curr->split_index];
